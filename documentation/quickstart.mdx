...
function RiskCheck() {
  ...
  useEffect(() => {
    fetch(`https://api.goes.health/v1/risks?location=37.7325,-119.5581`, {
      headers: {
...
async function checkRisks() {
  const response = await fetch(
    `https://api.goes.health/v1/risks?location=37.7325,-119.5581`,
    {
      headers: {
...
<CodeGroup>
```bash cURL
curl "https://api.goes.health/v1/risks?location=37.7325,-119.5581&ids=sunburn,high-altitude" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
function RiskCheck() {
  ...
  useEffect(() => {
    fetch(`https://api.goes.health/v1/risks?location=37.7325,-119.5581&ids=sunburn,high-altitude`, {
      headers: {
        'Authorization': `Bearer ${process.env.REACT_APP_GOES_API_KEY}`
      }
    })
...
<CodeGroup>
```bash cURL
curl -X POST "https://api.goes.health/v1/overview" \
...
function CompleteOverview() {
  ...
  useEffect(() => {
    fetch(`https://api.goes.health/v1/overview`, {
      method: 'POST',
      headers: {
